# Owner: backbone (ALL)
"""Unitree G1 (+ Robotiq 2F-85 grippers) platform description and low-level
controller.

Everything model-specific (joint names, rest posture, gripper actuators,
base mocap weld, gains, rate limits, IK chain, camera names) lives here or
in ``assets/g1_2f85_ee4705.xml`` (generated by scripts/build_g1_model.py)
— never in module code.  See ``assets/README.md`` for the documented
parameters.
...
Grippers
--------
Each 2F-85 has one upstream tendon-driven position actuator with control
range 0 (open) .. 255 (closed).  ``set_gripper(side, opening)`` stores a
target (1.0 = fully open, 0.0 = closed); ``update`` rate-limits the control
toward it.  ``gripper_opening()`` is the MEASURED opening derived from the
driver joints (0.8 rad = closed), same normalization.
...
# Robotiq 2F-85 grippers: prefix rg_ (right wrist) / lg_ (left wrist).  The
# eight linkage joints per gripper are NOT directly actuated: one tendon
# actuator drives the two driver joints (coupled by an equality).
GRIPPER_PREFIX = {"right": "rg_", "left": "lg_"}
_GRIPPER_JOINTS = ("right_driver_joint", "right_coupler_joint", "right_spring_link_joint", "right_follower_joint",
                   "left_driver_joint", "left_coupler_joint", "left_spring_link_joint", "left_follower_joint")
GRIPPER_JOINTS = {side: [p + j for j in _GRIPPER_JOINTS] for side, p in GRIPPER_PREFIX.items()}
GRIPPER_DRIVER_JOINTS = {side: [p + "right_driver_joint", p + "left_driver_joint"] for side, p in GRIPPER_PREFIX.items()}
GRIPPER_ACTUATORS = {side: p + "fingers_actuator" for side, p in GRIPPER_PREFIX.items()}
GRIPPER_PAD_GEOMS = {side: [p + n for n in ("right_pad1", "right_pad2", "left_pad1", "left_pad2")]
                     for side, p in GRIPPER_PREFIX.items()}
GRIPPER_PAD_BODIES = {side: [p + "right_pad", p + "left_pad"] for side, p in GRIPPER_PREFIX.items()}
GRIPPER_BASE_BODY = {side: p + "base" for side, p in GRIPPER_PREFIX.items()}
GRIPPER_CTRL_CLOSED = 255.0  # upstream ctrlrange 0..255
GRIPPER_DRIVER_CLOSED_RAD = 0.8  # driver joint range 0..0.8 (0 = open)
GRIPPER_MAX_OPENING_M = 0.085  # nominal 2F-85 stroke (pad gap when fully open)

PELVIS_BODY = "pelvis"
BASE_MOCAP_BODY = "base_target"
BASE_WELD = "base_weld"
RIGHT_EE_SITE = "rg_pinch"  # 2F-85 tool centre point (between the pads)
LEFT_EE_SITE = "lg_pinch"
RIGHT_EE_BODY = "rg_base"  # gripper base body carrying the pinch site (weld anchor)
...
# widened from 0.2 to 0.4 rad so the gripper clears the hip.
...
# RIGHT arm rest ("ready" posture): arm raised forward-right with the
# gripper ABOVE the table plane, out of the head camera's view and clear of
# the torso/hip (re-verified for the 2F-85 TCP, see docs/DECISIONS.md §12).
...
GRIPPER_REST = {j: 0.0 for side in GRIPPER_JOINTS for j in GRIPPER_JOINTS[side]}  # all linkage joints 0 = open
...
def rest_posture() -> dict[str, float]:
    """Joint -> value for every joint the controller holds at reset."""
    out: dict[str, float] = {}
    for d in (LEG_REST, WAIST_REST, LEFT_ARM_REST, RIGHT_ARM_REST, GRIPPER_REST):
        out.update(d)
    return out
...
class G1Limits:
    ...
    gripper_driver_speed: float = 2.0  # rad/s of driver-joint setpoint (full 0.8 rad stroke in 0.4 s)
...
# the base (pelvis-yaw) frame: approach axis (pinch site +z, along the
# forearm) pointing down and tilted APPROACH_TILT toward the azimuth
# direction; the pad-width axis (site +x) at FINGER_AZIMUTH left of the base
# heading, so the pads close perpendicular to that direction.  The old palm orientation is no longer
# collision-free with this flange/TCP; see docs/DECISIONS.md §12.
APPROACH_TILT = math.radians(45.0)  # was 20 deg with the palm TCP (docs/DECISIONS.md §12)
FINGER_AZIMUTH = math.radians(0.0)  # was 30 deg with the palm TCP
...
def opening_from_driver(q_driver: float) -> float:
    """Normalized gripper opening (1 open .. 0 closed) from a driver angle."""
    return float(np.clip(1.0 - q_driver / GRIPPER_DRIVER_CLOSED_RAD, 0.0, 1.0))
...
class G1Controller:
    def __init__(self, model: "mujoco.MjModel", data: "mujoco.MjData",
        ...
        for a in range(model.nu):
            if model.actuator_trntype[a] == mujoco.mjtTrn.mjTRN_JOINT:
                self._act_for_joint[model.joint(int(model.actuator_trnid[a, 0])).name] = a
        for j in rest_posture():
            if j not in self._act_for_joint and j not in GRIPPER_REST:
                raise ValueError(f"G1 model has no actuator for joint {j!r}")
        self._grip_act = {side: int(model.actuator(GRIPPER_ACTUATORS[side]).id) for side in GRIPPER_PREFIX}
        self._grip_driver_adr = {side: [self._qpos_adr[j] for j in GRIPPER_DRIVER_JOINTS[side]] for side in GRIPPER_PREFIX}
        self._grip_driver_dof = {side: [int(model.jnt_dofadr[model.joint(j).id]) for j in GRIPPER_DRIVER_JOINTS[side]]
                                 for side in GRIPPER_PREFIX}
        self._grip_target_ctrl = {side: 0.0 for side in GRIPPER_PREFIX}  # 0 = open
    ...
    def reset(self, x: float, y: float, yaw: float) -> None:
        ...
        jump), every joint actuator holding its joint's current value and
        both grippers open."""
        d = self.data
        ...
        for side, a in self._grip_act.items():
            d.ctrl[a] = 0.0
            self._grip_target_ctrl[side] = 0.0
    ...
    def stop_motion(self, *, preserve_right_grip: bool = False) -> None:
        """Cancel all commanded trajectories and hold measured joint poses.
        Only actuator/mocap targets change; never qpos, qvel or sim time.
        """
        self._cart_target = None
        self._cart_rot = None
        self._cart_rot_start = None
        self.last_arm_command = None
        self._ik_countdown = self._full_solve_countdown = self._stuck_steps = 0
        self._base_target[:] = self.base_pose()
        self._base_cmd[:] = self._base_target
        self.data.mocap_pos[self.mocap_id] = (*self._base_cmd[:2], STAND_HEIGHT)
        self.data.mocap_quat[self.mocap_id] = yaw_quat(self._base_cmd[2])
        self._arm_q_des = self.arm_q()
        self._waist_des = self.waist_q()
        self._write_hold_ctrl()
        openings = self.gripper_opening()
        for side, actuator in self._grip_act.items():
            if side == "right" and preserve_right_grip:
                continue  # keep contact force; stopping must not release a held body
            ctrl = (1.0 - openings[side]) * GRIPPER_CTRL_CLOSED
            self.data.ctrl[actuator] = ctrl
            self._grip_target_ctrl[side] = ctrl
    ...
    def set_gripper(self, side: str, opening: float) -> None:
        """Non-blocking gripper target: 1.0 = fully open, 0.0 = closed."""
        if side not in GRIPPER_PREFIX:
            raise ValueError(f"unknown gripper side {side!r}; expected 'left' or 'right'")
        if not math.isfinite(opening) or not 0.0 <= opening <= 1.0:
            raise ValueError(f"gripper opening must be in [0, 1], got {opening!r}")
        self._grip_target_ctrl[side] = (1.0 - float(opening)) * GRIPPER_CTRL_CLOSED

    def gripper_target(self, side: str) -> float:
        return 1.0 - self._grip_target_ctrl[side] / GRIPPER_CTRL_CLOSED

    def gripper_opening(self) -> dict[str, float]:
        """MEASURED opening of both grippers (mean driver angle -> [0, 1])."""
        out = {}
        for side, adrs in self._grip_driver_adr.items():
            q = float(np.mean([self.data.qpos[a] for a in adrs]))
            out[side] = opening_from_driver(q)
        return out

    def gripper_driver_speed(self, side: str) -> float:
        return float(np.max(np.abs([self.data.qvel[i] for i in self._grip_driver_dof[side]])))
    ...
    def set_arm_target(self, pos_world: np.ndarray, rot_world: Optional[np.ndarray] = None) -> IKResult:
        """Non-blocking IK target; raises only when no solution exists.
        Reissuing the current target preserves its active stream."""
    ...
    def update(self) -> None:
        ...
        # grippers: control-unit rate limit equivalent to gripper_driver_speed
        max_step = lim.gripper_driver_speed / GRIPPER_DRIVER_CLOSED_RAD * GRIPPER_CTRL_CLOSED * dt
        for side, a in self._grip_act.items():
            d.ctrl[a] = float(np.clip(self._grip_target_ctrl[side], d.ctrl[a] - max_step, d.ctrl[a] + max_step))
        # legs, left arm: held at their reset values (unchanged ctrl)
    ...
    def ee_rot_at(self, q: np.ndarray) -> np.ndarray:
        """EE site rotation the arm WOULD have at joint values q (scratch)."""
        self._scratch.qpos[:] = self.data.qpos
        _pos, rot = self.ik._fk(self._scratch, np.asarray(q, dtype=float))
        return rot
