#!/usr/bin/env bash
# Like train.sh, plus EVAL_TASK=place (checkpoint VAL = full-executor PLACE episodes). Bonus 8.3:
# train one lerobot policy (scripts/bonus/train_policy.py, fast data path)
...
.venv/bin/python scripts/bonus/watch_ckpts.py --run "$out" --policy "$kind" --name "$name" --last-step "$steps" \
    --train-pid "$pid" --workers "${EVAL_WORKERS:-3}" --device "${EVAL_DEVICE:-cpu}" --task "${EVAL_TASK:-grasp}" >> "runs/bonus/logs/$name.watch.log" 2>&1 &
wpid=$!
set +e
